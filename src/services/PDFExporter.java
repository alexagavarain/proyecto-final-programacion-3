...
public class PDFExporter {
	...
	public void exportUsers(List<User> users, File file) throws IOException {
		...
		try (PdfDocument pdfDoc = new PdfDocument(new PdfWriter(file));
			...
			float[] columnsWidth = { 1, 3, 4, 3, 2, 2 };

			Table table = new Table(UnitValue.createPercentArray(columnsWidth)).useAllAvailableWidth();

			PdfFont font = PdfFontFactory.createFont(StandardFonts.HELVETICA);

			Cell cell = new Cell(1, 6).add(new Paragraph("Usuarios del sistema")).setFont(font).setFontSize(14)
			...
			for (int i = 0; i < 2; i++) {
				...
				Cell[] headerFooter = new Cell[] {
						...
						new Cell().setTextAlignment(TextAlignment.CENTER).setBorderTop(new SolidBorder(1f))
								.setBackgroundColor(new DeviceGray(0.80f)).add(new Paragraph("Carrera")),

						new Cell().setTextAlignment(TextAlignment.CENTER).setBorderTop(new SolidBorder(1f))
								.setBackgroundColor(new DeviceGray(0.80f)).add(new Paragraph("Turno")),
								
						new Cell().setTextAlignment(TextAlignment.CENTER).setBorderTop(new SolidBorder(1f))
					    .setBackgroundColor(new DeviceGray(0.80f)).add(new Paragraph("Grupo")),};
