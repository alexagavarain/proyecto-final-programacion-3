...
public class UserFormDialogController {
	...
	private void saveUser() {
		if (validateEdit()) {
			...
			User newUser = new User(name, email, password, "Usuario", group);
			
			try {
				repo.save(newUser);
				JOptionPane.showMessageDialog(view.getContentPane(), "Usuario agregado correctamente");
				view.dispose();
			} catch (Exception e) {
				e.printStackTrace();
				JOptionPane.showMessageDialog(view.getContentPane(), "Error al guardar el usuario");
			}
		} else {
	...
	private void loadCareers() {
		...
		for(Career career : careers) {
			...
			if(user != null && user.getGroup() != null && user.getGroup().getCareer() != null) {
